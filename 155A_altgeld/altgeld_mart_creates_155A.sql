...
drop table if exists a_emp.location_types;
use a_emp;



create table a_emp.location_types(
    loc_type            varchar(25) not null
, constraint loc_type_pk  primary key(loc_type)
)engine = INNODB;
...
create table a_emp.locations(
  ...
  , constraint loc_loc_type_fk   foreign key (loc_type) 
        references  a_emp.location_types(loc_type)
